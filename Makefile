...
go-test-nat: go-get
	$(FLOCK) firewall.lock go test -v -exec sudo $(go.module)/lib/nat
...
go-test-other: go-get test-cluster
	KUBECONFIG=$(abspath ${KUBECONFIG}) $(FLOCK) cluster.lock go test -v $$(go list ./... | grep -vF -e $(go.module)/lib/nat -e $(go.module)/cmd/teleproxy)
